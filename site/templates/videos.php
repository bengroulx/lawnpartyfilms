    ...
    <div class="c-contain">
        ...
        <div class="c-video-grid u-flexbox a-fade-in">
            ...
            <?php foreach ($page->children()->listed() as $subpage) : ?>
                <a class="c-video-grid__item u-context u-clip u-bg-cover u-bg-center u-white u-align-left u-flexbox js-lazy" href="<?php echo $subpage->url() ?>" <?php if ($subpage->coverImage()->isNotEmpty()) : ?> data-src="content/<?= $subpage->diruri() ?>/<?php echo $subpage->coverImage() ?>" <?php else : ?> data-src="<?php echo $subpage->image()->resize(500)->url() ?>" <?php endif ?>>
